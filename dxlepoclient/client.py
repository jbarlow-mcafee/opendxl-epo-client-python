...
class OutputFormat(object):
    ...
    def validate(output_format):
        ...
        """
        if output_format not in[OutputFormat.JSON, OutputFormat.XML, OutputFormat.VERBOSE, OutputFormat.TERSE]:
            raise Exception("Invalid output format: {0}".format(output_format))


class EpoClient(object):
    """
    This client provides a high level wrapper for invoking ePO remote commands via the
    ...
    def __init__(self, dxl_client, epo_unique_id=None):
        ...
        **ePO Unique Identifier**
            ...
            DXL supports communicating with multiple ePO servers on a single DXL fabric. However,
            each instance of this client can only be associated with one ePO server
            (the server it will be invoking remote commands on).

            The ePO unique identifier specified must match the identifier that was associated with the
            particular ePO when a corresponding ePO DXL service was started.

            If only one ePO server is connected to the DXL fabric this parameter is optional
            (the client will automatically determine the ePO's unique identifier).

            The :func:`lookup_epo_unique_identifiers` method can be used to determine the unique
            identifiers for ePO servers that are currently exposed to the fabric.

        Constructor parameters:

        :param dxl_client: The DXL client to use for communication with the ePO DXL service
        :param epo_unique_id: (optional) The unique identifier used to specify the ePO server that this
        ...
        if epo_unique_id is None:
            ...
            epo_ids = self.lookup_epo_unique_identifiers(self._dxl_client, self._response_timeout)

            epo_ids_len = len(epo_ids)
            if epo_ids_len is 1:
                epo_unique_id = next(iter(epo_ids))
            elif epo_ids_len is 0:
                raise Exception("No ePO DXL services are registered with the DXL fabric")
            else:
                raise Exception(("Multiple ePO DXL services are registered with the DXL fabric ({0}). "
                                 "A specific ePO unique identifier must be specified.").format(", ".join(epo_ids)))
    ...
    def run_command(self, command_name, params=None, output_format=OutputFormat.JSON):
        """
        ...
        **Example Response**
        ...
        :param output_format: (optional) The output format for ePO to use when returning the response.
    ...
    def _sync_request(dxl_client, request, response_timeout, payload_dict):
        ...
        """
        # Set the payload
        request.payload = json.dumps(payload_dict).encode(encoding=EpoClient._UTF_8)
    ...
    @staticmethod
    def lookup_epo_unique_identifiers(dxl_client, response_timeout=DEFAULT_RESPONSE_TIMEOUT):
        """
                ...
                if "requestChannels" in service:
                    ...
                    for channel in channels:
                        if channel.startswith(EpoClient.DXL_REQUEST_PREFIX):
                            ret_ids.add(channel[len(EpoClient.DXL_REQUEST_PREFIX):])
        return ret_ids
